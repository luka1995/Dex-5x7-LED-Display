...
MAIN
	...
	; Enable RS232
		banksel SPBRG
		clrf SPBRGH						; set 25 for 19,2kbps
		movlw .25						;
		movwf SPBRG						;

		banksel RCSTA					;
		movlw b'10010000'				; set 8bit and enable RX
		movwf RCSTA						;

		banksel TXSTA					;
		movlw b'00100100'				; set high bit rate & 8 bit
		movwf TXSTA						;

		;;;

		movlw .0						; adress 0
		call READ_EEPROM				; read eeprom address
		movwf DISPLAYPOSITION			; save it to display position
...
MAIN_LOOP
	banksel PIR1
	bcf RCSTA, OERR					; clear error bit (we assume everything went OK)

	btfss PIR1, RCIF				; test if new character recived
	goto MAIN_LOOP					;

	; character recived

	movfw RCREG						; read char
	movwf READED_CHAR				; save it

	movlw .27						; set 0 characted to ESC
	subwf READED_CHAR, w			;
	btfsc STATUS, Z					; if we read 0
	goto ZERO						; goto 0

	; not counting control BYTES

	decfsz DISPLAYPOSITION			; decrement DISPLAYPOSITION and if nit zero
	goto MAIN_LOOP					;

	; counting control BYTES

	movfw READED_CHAR				; read character
	pagesel DISPLAY_ASCII			; and display it to ASCII
	call DISPLAY_ASCII				; Display


	goto MAIN_LOOP

	ZERO								; reset character count!
	movlw .0						; adress 0
	call READ_EEPROM				; read eeprom address
	movwf DISPLAYPOSITION			; save it to display position
	goto MAIN_LOOP					;

	; Reads data from EEPROM. Addres containd in W, result returned in W
	READ_EEPROM
	banksel EEADR 					;
	movwf EEADR 					; from Data Memory
	banksel EECON1					;
